...
int deadSnake = 0; //setting variable for identifying dead snake
...
void moveSnake2() {
    // Simple AI: stick to the chosen apple until it is eaten
    int newDirX = dir2X;
    int newDirY = dir2Y;

    if (apple2X != snake2X[0]) {
    ...
    for (int i = 0; i < numSegments2; i++) {
        if (newX == snake2X[i] && newY == snake2Y[i]) {
            collision = true;
            break;
        }
    }

    // If a collision is detected, try turning right or left
    if (collision) {
        // Try turning right
        int temp = newDirX;
        newDirX = -newDirY;
    ...
    if (newX < tft.width() / 2 || newX >= tft.width() || newY < 0 || newY >= tft.height()) {
        ...
        if (newX < tft.width() / 2 || newX >= tft.width() || newY < 0 || newY >= tft.height()) {
            // If still out of bounds, try turning into the other direction
            newDirX = -newDirX;
            newDirY = -newDirY;
        }
    }
...
void showLevelScreen()
{
    ...
    tft.println(" ");
    tft.println("SNEC ver 2.3");
    tft.println("Add: level selector");
    tft.println("Add: proper death screen");
